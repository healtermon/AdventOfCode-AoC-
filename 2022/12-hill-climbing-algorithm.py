...
def scalable(df,c,n): return height(df.iloc[n[R],n[C]]) - height(df.iloc[c[R],c[C]]) <= 1
...
def unrecorded(v,e,l): return [n for n in l if not (posNode(n) in posList(v) or posNode(n) in posList(e))]
...
while edges:
    curr = popMin(edges)
    visited.append(curr)
    edges.extend(traversable(e,curr,unrecorded(visited,edges,adjacent(curr))))
